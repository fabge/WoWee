...
- `wowee_input` — `src/core/input.cpp`. SDL key and mouse state for this frame. It depends on nothing of ours and is read by ui, rendering and core alike; in `wowee_core` it made a device abstraction look like a dependency on the composition root.
...
The subsystem libraries are declared as a cycle, because the symbol graph is still one — 2 mutual pairs as of 2026-08-27, down from 18. Measure them with `tools/library_cycle_check.py <build-dir>` rather than by hand. CMake allows cycles among static libraries and repeats the connected component. `TODO.md` ranks what is left by its weakest side. Do not replace the cycle with a hand-written edge list until they are gone: the measurement is macOS-only, a missing edge is a link failure on GNU ld, and CI builds five platforms.
...
| `tools/validate.sh --quick` (185 tests, no `sweep_guard`) | ~15s |
